...
use App\Services\Tasks\TaskDurationCalculatorService;
    ...
    public function infolist(Infolist $infolist): Infolist
    {
        ...
        return $infolist->schema([
            ...
            Section::make('ملفات المهمة')
                ->schema([
                    TextEntry::make('agreement_file')
                        ->label('ملف الاتفاقية')
                        ->html()
                        ->visible(fn () => $h->canViewAgreementFile(auth()->user()))
                        ->state(function (ProductionTask $record) {
                    ...
                    TextEntry::make('additional_work_file_link')
                        ...
                        ->html()
                        ->visible(fn () => $h->canViewAdditionalWorkFile(auth()->user()))
                        ->state(function (ProductionTask $record) {
                            $pr = $record->project?->productionRequest;
                    ...
                    TextEntry::make('client_receipt_link')
                        ...
                        ->html()
                        ->visible(fn () => $h->canViewClientReceipt(auth()->user()))
                        ->state(function (ProductionTask $record) {
            ...
            Section::make('مدد مراحل التصنيع')->schema([
                ViewEntry::make('stage_durations')
                    ->label(false)
                    ->view('filament.task.duration-table')
                    ->state(fn(ProductionTask $record) => app(TaskDurationCalculatorService::class)->calculatePhases($record))
                    ->columnSpanFull()
            ])->columnSpanFull(),
            ...
            ])->columns(1)->visible(fn () => $h->canViewPurchasingSection(auth()->user())),
